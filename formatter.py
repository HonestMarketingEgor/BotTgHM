...
def build_help_text(bot_username: str | None = None) -> str:
    ...
    return "\n".join(
        [
            "ИИ-ассистент для рекламного маркетингового агентства.",
            "",
            "Главная польза:",
            "— быстрый анализ переписки и решений по проектам",
            "— гипотезы и следующие шаги для маркетинга",
            "— понимание, каких данных не хватает для решения",
            "",
            "Ключевые возможности:",
            "— анализ текущего чата: /ask <вопрос>",
            "— ручная сводка за период: /daily_summary [часы]",
            "— персональный разбор ссылки: отправь URL и бот даст конкретные правки",
            "— анализ другого чата по названию (если у тебя есть доступ к обоим чатам)",
            "— сопоставление лидов из файлов: /vkmatch",
            "— хранение оперативного контекста до 6 месяцев",
            "— корректное название и id текущего чата: /chat_info",
            "",
            "Быстрые команды:",
            "— /help",
            "— /ask <вопрос>",
            "— /daily_summary [часы] (или /summary [часы])",
            "— /mode assistant | /mode analysis",
            "— /chat_info",
            "— /vkmatch",
            "— /reset",
            "",
            ...
            "После ответа бота есть короткое окно контекста: можно продолжать без @mention.",
        ]
    )
